...
def pull_type(type_name:str):
    ...
    local_db.commit()
    r = session.get(f"https://arxiv.org/list/{type_name}/recent?skip=0&show=5000",verify=False).content.decode('utf-8')
    # <a href="/abs/2403.02285" title="Abstract">arXiv:2403.02285</a>
    entries = re.findall(r'/abs/(\d+\.\d+)" title="Abstract"',r)
    if(len(entries)==0):
        return []
    # iterate through the entries every 10 entries
    res = []
    while(len(entries) > 0):
